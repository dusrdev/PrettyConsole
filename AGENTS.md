...
- Coloring model
  ...
  - `ConsoleColor` interpolation remains supported for compatibility, but explicit `ConsoleColor` APIs are now primarily for low-level writes and components like `ProgressBar`, `Spinner`, `Console.SetColors`, and `TypeWrite`.
